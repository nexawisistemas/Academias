...
export async function updateOrganizationAction(data:FormData){const{db,organizationId}=await scope();const{data:current}=await db.from("organization_settings").select("branding").eq("organization_id",organizationId).single();const lines=(key:string)=>text(data,key).split("\n").map(item=>item.trim()).filter(Boolean);const structured=(key:string,fields:string[])=>lines(key).map(line=>{const parts=line.split("|").map(item=>item.trim());return Object.fromEntries(fields.map((field,index)=>[field,parts[index]||""]));});const branding={...((current?.branding||{}) as Record<string,unknown>),logo_url:text(data,"logo_url")||null,eyebrow:text(data,"eyebrow")||null,headline:text(data,"headline")||null,headline_accent:text(data,"headline_accent")||null,description:text(data,"description")||null,primary_color:text(data,"primary_color")||"#5dff9f",instagram:text(data,"instagram")||null,hours:lines("hours"),modalities:structured("modalities",["title","description"]),team:structured("team",["name","role","specialty"]),seo_title:text(data,"seo_title")||null,seo_description:text(data,"seo_description")||null};await db.from("organizations").update({name:text(data,"name"),phone:text(data,"phone")||null,email:text(data,"email")||null}).eq("id",organizationId);await db.from("organization_settings").update({branding}).eq("organization_id",organizationId);revalidatePath("/dashboard/configuracoes");revalidatePath("/dashboard");revalidatePath(`/academia/${text(data,"slug")}`);}
